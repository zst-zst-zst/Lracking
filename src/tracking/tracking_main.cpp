...
// measurement.h 已删除 (cb3fbca), 改用 common/types.h + detector.h 直接取定义
    ...
    std::thread control_thread([&]() {
        ...
        while (!g_should_exit.load()) {
            ...
            while (now >= next_send) {
                ...
                if (use_meas.valid && use_meas.bbox_height > 2.0f) {
                    constexpr double kRadToDeg = 180.0 / M_PI;
                    const double z_est = cam_model.fy * ctrl_cfg.target_height_m / use_meas.bbox_height;
                    if (z_est > 1e-6) {
    ...
    while (true) {
        ...
        try {
            ...
            if (!dets.empty()) {
                ...
                // 原 detect::toMeasurement(*best_det, ts) 内联展开 (该函数已弃用):
                meas.valid = best_det->valid;
                meas.timestamp = ts;
                meas.uv = best_det->center;
                meas.confidence = best_det->confidence;
                meas.class_id = best_det->class_id;
                meas.label = best_det->label;
                meas.bbox_area = static_cast<float>(best_det->bbox.width) * best_det->bbox.height;
                meas.bbox_height = static_cast<float>(best_det->bbox.height);
            }
            // ── 延迟补偿: 估计像素速度 (px/s) ──
